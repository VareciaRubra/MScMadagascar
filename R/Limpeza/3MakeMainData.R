...
current.data <- Sp.raw.main.data$Eulemur_albifrons
makeMainData <- function (current.data, specie = TRUE, compare.size = FALSE, final = FALSE ) 
  ...
  {x[[12]] <- if(x[[9]]>40) ldply(.data = x[[11]][-c(2,5,6)], .fun = MeanMatrixStatistics, parallel = TRUE, full.results = FALSE) else NA
   ...
   x[[14]] <- if(x[[9]]>15) RandomSkewers(cov.x = x[[11]][c(1,3,4)], num.vectors = 1000, repeat.vector = unlist(x[[13]])[c(1,3,4)]  ) else NA
  }   
...
save( Strepsirrhini.image,
     file = "~/atachesStrepsirrhini_image_2.RData")
