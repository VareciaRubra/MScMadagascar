...
extant.main.data$All$info$Familia <- factor (extant.main.data$All$info$Familia, levels = unique(extant.main.data$All$info$Familia) )
extant.main.data$All$info$Genero <- factor (extant.main.data$All$info$Genero, levels = unique(extant.main.data$All$info$Genero) )
...
#resp %<>% mutate(., PC1 = PRCOMP$scores[,1], PC2=PRCOMP$scores[,2]) 
Wmat = CalculateMatrix(manova(as.matrix(extant.main.data$All$ed)  ~ Especie, data = as.data.frame(extant.main.data$All$info) ) )
resp <- as.data.frame(as.matrix(na.omit(select(current.data, IS_PM:BA_OPI) ) ) %*% eigen(Wmat)$vectors[,c(1,4)])
resp %<>% mutate(., ID = rownames(resp) )
names(resp) <- c("PC1", "PC4", ".id")
resp.info <- select(lin_data, c(.info,Arquivo:Data_dado) )
resp.info <- resp.info[resp.info$Tombo %in% resp$.id, ]
plot.W <- cbind(resp.info, resp)
...
#              aes(PC1, PC4, label= Tombo), size = 3) 
#   geom_point(data = ddply(plot.W, .(Genero), numcolwise(mean)),
#             aes(PC1, PC4, group= Genero), size = 3) +
